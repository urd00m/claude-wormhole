...
/**
 * Env for the resident worker's CLI subprocess. Like one-shot workers we
 * bump the async-agent stall watchdog, but resident workers sit IDLE
 * between invocations by design, so we push it well past the one-shot
 * value — an idle resident worker waiting for its next prompt must not be
 * reaped by the watchdog. 24h unless the user overrides.
 */
function buildResidentEnv(): Record<string, string> {
