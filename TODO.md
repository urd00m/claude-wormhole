...
- [ ] **Remove `claude -p` usage everywhere — go through the SDK** — only one site left: `src/mcp/codexSpawnServer.ts:78,130` (the codex→claude leaf path), which currently does `spawn("claude", ["-p", "--output-format", "json", "--dangerously-skip-permissions", ...])` and parses the printed JSON. Replace with the same `query()` from `@anthropic-ai/claude-agent-sdk` everything else in the bot already uses, so we go through one auth/MCP/canUseTool surface. Why it's `-p` today: the codex spawn server runs as a separate stdio process under `codex exec`, outside the wormhole node process, and the SDK looked heavy for a one-shot leaf call — but the bot has had the SDK as a dep all along, and `-p` cuts us off from progress events, structured tool calls, and the same `getContextUsage`/usage metering the rest of the bot relies on. Migration: import `query` in `codexSpawnServer.ts`, replace `buildClaudeSubArgs` + `spawn` with a `query({ prompt, options: { ... } })` loop that drains `assistant`/`result` messages, then drop the `--output-format json` parsing. Touches `runCodexWorker`/`makeRealRunner`'s claude branch; the codex branch (`codex exec`) stays as a subprocess since there's no SDK alternative. Test path: existing `src/integration/spawnChain.it.ts` covers codex→claude.
...
- [ ] **Caveman statusline badge** — wire up the `[CAVEMAN]` / `[CAVEMAN:ULTRA]` status indicator that ships with the caveman plugin. Add to `~/.claude/settings.json`: `"statusLine": { "type": "command", "command": "bash \"/Users/pixelstealing/Desktop/claude-wormhole/arch-common/caveman/hooks/caveman-statusline.sh\"" }`. Hook script already present at `arch-common/caveman/hooks/caveman-statusline.sh`; just needs the settings entry to render in the CLI status bar.
