...
- [x] **Caveman statusline badge** — `[CAVEMAN]` / `[CAVEMAN:ULTRA]` indicator wired: `statusLine` entry added to `~/.claude/settings.json`. Note the upstream vendoring never actually included a statusline script (the earlier claim that it was "already present" was wrong) — `arch-common/caveman/hooks/caveman-statusline.sh` is ours, resolving the mode from `CAVEMAN_DEFAULT_MODE` env → `~/.claude/.caveman-active` flag → `data/cavemanState.json`.
